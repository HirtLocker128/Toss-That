...
*/
public class NextIceCube : MonoBehaviour {
    /*
    ...
    */
    void Awake () {
        substate = NextIceCube.state.none;
        nextGameObject = null;
        ...
        //next ice obj
        gameObject.GetComponent<NextIceCube>().enabled = false;
    }
    ...
    */
    void Update () {
        if (substate == NextIceCube.state.handled)
        {
            //prepare to remove all the scripts from the currently tossed ice cube, and
            //move it to the next ice cube.
            nextGameObject.AddComponent<Tosser>();
            nextGameObject.AddComponent<IceCollider>();
            nextGameObject.AddComponent<NextIceCube>();
            
            //no logic should come after this code block and after the if-elseif's here.
            Destroy(gameObject.GetComponent<Tosser>());
            Destroy(gameObject.GetComponent<IceCollider>());

            substate = NextIceCube.state.none;
            nextGameObject = null;
            //We destroy this script
            Destroy(gameObject.GetComponent<NextIceCube>());
        }
        else if (substate == NextIceCube.state.success)
        {
            //add to the score.
            ...
            substate = NextIceCube.state.handled;
        }
        else if (substate == NextIceCube.state.fail)
        {
        ...
        else if (substate == NextIceCube.state.none)
        {

        }
        else if (substate == NextIceCube.state.pause)
        {
            
            substate = NextIceCube.state.paused;
        }
        else if (substate == NextIceCube.state.paused)
        {

        }
        else if (substate == NextIceCube.state.unpause)
        {
            substate = savedState;
        }
    }
    ...
    */
    public void successThrow()
    {
        ...
        substate = NextIceCube.state.success;
    }
    public void failThrow()
    {
        ChancesLeft.decrementChance();
        substate = NextIceCube.state.fail;
    }
    /*
    ...
    */
    void switchIceCubes()
    {
		if (GameObject.Find ("Table")) {
			...
			Vector3 outOfView = GameObject.Find ("Table").GetComponent<CubeRestorer> ().getOutOfView (currIceIndex);
			...
			//GameObject.Find ("pauseButton").GetComponent<Paused> ().updateIceCube (nextGameObject);
			GameObject.Find ("Table").GetComponent<Melter> ().updateIceCube (nextGameObject);

			//We set the new ice cube to the tossable position and orientation.
			Vector3 toss = GameObject.Find ("Table").GetComponent<CubeRestorer> ().getTossPos ();
			nextGameObject.transform.position = toss;

			nextGameObject.transform.rotation = GameObject.Find ("Table").GetComponent<CubeRestorer> ().getTossOri ();
    ...
    */
    void increaseIndex()
    {
        ...
        if (currIceIndex == 14)
        {
            for (int i = 0; i < 15; i++)
            {
                ...
                iObj.transform.position = GameObject.Find("Table").GetComponent<CubeRestorer>().getPlatePos(i);
            ...
            GameObject.Find("Table").GetComponent<Melter>().temperatureIncreases();
            currIceIndex = 0;
        }
        else
        {
    ...
    {
        savedState = substate;
        substate = NextIceCube.state.pause;
    }
    public void unpauseGame()
    {
        substate = NextIceCube.state.unpause;
    }
}
